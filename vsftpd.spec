...
Patch15: vsftpd-2.2.2-clone.patch
Patch16: vsftpd-2.2.2-v6only.patch
Patch17: vsftpd-2.2.2-isolate.patch
Patch18: vsftpd-2.2.2-cve2011-0762.patch
Patch19: vsftpd-2.2.2-aslim.patch
Patch20: vsftpd-2.2.2-sqb.patch
Patch21: vsftpd-2.2.2-uint-uidgid.patch
Patch22: vsftpd-2.2.2-conn_fail.patch
Patch23: vsftpd-2.2.2-lookup.patch
Patch24: vsftpd-2.2.2-defval.patch
Patch25: vsftpd-2.2.2-sitehelp.patch
Patch26: vsftpd-2.2.2-logging.patch
...
%patch15 -p1 -b .clone
%patch16 -p1 -b .v6only
%patch17 -p1 -b .isolate
%patch18 -p1 -b .cve2011-0762
%patch19 -p1 -b .aslim
%patch20 -p1 -b .sqb
%patch21 -p1 -b .uint-uidgid
%patch22 -p1 -b .conn_fail
%patch23 -p1 -b .lookup
%patch24 -p1 -b .defval
%patch25 -p1 -b .sitehelp
%patch26 -p1 -b .logging
...
* Tue Jul 29 2014 Jiri Skala <jskala@rehat.com> - 2.2.2-13
- Resolves: #921976 - vsftpd server fails during large ASCII file transfer over SSL

* Tue Feb 12 2013 Jiri Skala <jskala@rehat.com> - 2.2.2-12
- Resolves: #903147 - connect rigth socket when the data are transferred between two servers

* Fri Mar 02 2012 Jiri Skala <jskala@rehat.com> - 2.2.2-11
- Resolves: #785084 - improved: vsftpd can not handle square brackets in ls

* Mon Feb 13 2012 Jiri Skala <jskala@rehat.com> - 2.2.2-10
- Resolves: #745133 - corrected doc for 'max_per_ip' and 'max_clients' default setting

* Thu Feb 02 2012 Jiri Skala <jskala@rehat.com> - 2.2.2-9
...
* Mon Dec 12 2011 Jiri Skala <jskala@rehat.com> - 2.2.2-8
- Resolves: #708657 - Update to rhel 6.1 broke vsftpd authentication when using ldap

* Thu Mar 03 2011 Jiri Skala <jskala@rehat.com> - 2.2.2-7
- Resolves: #681892 - CVE-2011-0762 vsftpd: remote DoS via crafted glob patter

* Wed May 26 2010 Jiri Skala <jskala@rehat.com> - 2.2.2-6
- Resolves: #595617 - options "isolate" & "isolate_network" not documented

* Mon May 17 2010 Jiri Skala <jskala@rehat.com> - 2.2.2-5
- Resolves: #592850 - vsftp binds to both ipv4 and ipv6 addresses when listen_ipv6 is enabled 

* Thu May 13 2010 Jiri Skala <jskala@rehat.com> - 2.2.2-4
- Resolves: #548802 - vsftpd gets sig=11 when user attempts to login

* Tue Mar 16 2010 Jiri Skala <jskala@rehat.com> - 2.2.2-3
- Resolves: #540818 - Local user access denied by default vsftpd.conf
- Resolves: #580521 - vsftpd issues in RHEL5.4 (cloned)

* Tue Mar 16 2010 Jiri Skala <jskala@rehat.com> - 2.2.2-2
- Resolves: #574038 - /etc/init.d/vsftpd does not start more than one daemon

* Thu Jan 14 2010 Jiri Skala <jskala@rehat.com> - 2.2.2-1
- Resolves: #555321 - vsftpd segfaults with passive mode and SSL
- updated to latest upstream version - fixes the bug #555321

* Mon Nov 23 2009 Dennis Gregorovic <dgregor@redhat.com> - 2.2.0-5.1
- Rebuilt for RHEL 6
